    ...
    useEffect(() => {
        const fetchEmployees = async () => {
            try {
                ...
                // Filter only active employees if needed, but for history maybe all?
                // setEmployees(data.filter(emp => emp.status === 'Active'));
                setEmployees(data); // Show all for now to debug
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                employee: formData.expenseType === 'Employee' ? formData.employee : null,
                details: formData.expenseType === 'Employee' ? formData.details : {}
            };
